            ...
            foreach(INamedTypeSymbol classEntry in classMethods.Keys)
            {
                ...
                foreach (IMethodSymbol method in classMethods[classEntry])
                {
                    ...
                    foreach(string part in docParts)
                    {
                        ...
                        if (line.Contains("///") == false && line.Contains(taskSymbol.ToString()) == false) line = "    /// " + line;
                        if (line.Trim() != "///")
                        {
                            doc.AppendLine(line);
                            if (inSummary == true && line.Contains("summary>") == false)
                            {
                                summary.Append(line);
                    ...
                    if (returnsAdded == false)
                    {
                        string[] sumParts = summary.ToString().Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
                        stringBuilder.AppendLine("        /// <returns>");
                        stringBuilder.AppendLine("        /// Task which ");
                        sumParts[0] = sumParts[0].Replace("/// ", "").Trim();
                        sumParts[0] = char.ToLower(sumParts[0][0]) + sumParts[0].Substring(1);
                        foreach(string sp in sumParts)
                        {
                            stringBuilder.Append("        /// ");
                            stringBuilder.AppendLine(sp.Replace("///", "").Trim());
                        }
                        stringBuilder.AppendLine("        /// </returns>");
                    }
                    stringBuilder.Append("        ");
